...
public class PushAgentBasic : Agent
{
	...
	public override List<float> CollectState()
	{
		...
		Vector3 obstaclePosRelToAgent = obstacle.transform.position - agentRB.position; //vector to obstacle from agent
		...
		MLAgentsHelpers.CollectVector3State(state, obstaclePos);  //pos of obstacle rel to ground
	...
	{
		reward += 5; //i chose 5 because a reward signal of 1 wasn't strong enough
	...
	{
		reward -= 5f; //i chose 5 because a penalty signal of 1 wasn't strong enough
		done = true; //if we mark an agent as done it will be reset automatically. AgentReset() will be called.
		StartCoroutine(GoalScoredSwapGroundMaterial(academy.failMaterial, 1)); //swap ground material for a bit to indicate we failed. we are a failure.
	...
	public void MoveAgent(float[] act) 
	{
        ...
        if (brain.brainParameters.actionSpaceType == StateType.continuous)
        {
			...
			float energyConservPenaltyModifier = 10000; //lower the number, the more penalty it receives for movement.
			if(act[0] != 0) //left/right movement
			{
				reward -= Mathf.Abs(act[0])/energyConservPenaltyModifier; //the larger the movement, the more penalty given.
			}
			if(act[1] != 0) //forward/back movement
			{
				reward -=  Mathf.Abs(act[1])/energyConservPenaltyModifier;//the larger the movement, the more penalty given.
			}
